...
ALTER TABLE ib_notes
ADD COLUMN IF NOT EXISTS exam_importance text DEFAULT 'Core Concept';

-- Update any existing rows to have the default if null or empty
UPDATE ib_notes 
SET exam_importance = 'Core Concept' 
WHERE exam_importance IS NULL OR exam_importance = 'Medium';

-- Optional: Modify default for future inserts
ALTER TABLE ib_notes
ALTER COLUMN exam_importance SET DEFAULT 'Core Concept';
